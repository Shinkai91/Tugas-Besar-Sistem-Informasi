...
<body style="height:100%">
    <nav class="navbar navbar-light sticky-top justify-content-between" id="navbar">
        <a class="navbar-brand font-weight-bold" style="color:#3C4858;" data-aos="fade-right" data-aos-duration="3000">Rental <span class="text-info">Laptop</span></a>
    ...
    <div class="container mt-5">
        <form method="POST" action="/admin/laptop">
            ...
            <div class="row">
                <div class="col">
                    <button type="submit" class="btn btn-primary btn-custom1 mr-1 mt-2 mb-4" style="width:70px;height:35px;">Add</button>
                </div>
            </div>
        </form>
    </div>
            ...
            <table class="table table-hover">
                <tr  class="text-center">
                  ...
                  <tr class="text-center">
                    ...
                    <td>{{ $data->status }}</td>
                    <td>{{ $data->note }}</td>
                    @foreach ($data_operator as $key => $operator)
                    @if ($data->id_operator === $operator->id)
                    <td>{{ $operator->nama }}</td>
                    @else
                    <td></td>
                    @endif
                    ...
                    <td>
                      ...
                      <a class="btn btn-danger" style="width:70px"  href="/admin/laptop/delete/{{ $data->id }}">Hapus</a>
                    </td>
                  </tr>
                  @endforeach
            </table>
